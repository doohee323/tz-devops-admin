...
class Controller:
    ...
    def __init__(self, env=()):
        ...
        processor = BatchSpanProcessor(OTLPSpanExporter(endpoint=env.get('exporter_otlp_traces_endpoint')))
    ...
    def do_POST(self, httpd):
        self.tracer = trace.get_tracer("do_POST")
        logger.info(f"self.path: {httpd.path}")
        with self.tracer.start_as_current_span(httpd.path) as span:
            if httpd.path == '/awsri':
    ...
    def get_s3(self, s3repo='', exec_type='GET'):
        ...
        if exec_type == 'POST':
            ...
            with self.tracer.start_as_current_span("get_s3") as span:
    ...
    def get_ri(self, profile='', region='', type='', exec_type='GET'):
        ...
        out = ''
        with self.tracer.start_as_current_span("get_ri") as span:
            if exec_type == 'POST':
                return out
            elif exec_type == 'GET':
                if type == 'db':
    ...
    def ri_cal(self, ri_cmd, usage_cmd):
        ...
        with self.tracer.start_as_current_span("ri_cal") as span:
            ...
            for out1 in iter(process.stdout.readline, b''):
                ...
                RIArry.append(RI)
            span.set_attribute("printed_string", "1. get RI infos")
            ...
            for out1 in iter(process.stdout.readline, b''):
                ...
                if len(UseArry) == 0:
                    ...
                    if chk is False:
                        UseArry.append(Use)
            span.set_attribute("printed_string", "2. get Current usage")
            ...
            span.set_attribute("printed_string", "3. RIArry - UseArry")
    ...
    def get_vpcs(self, profile='', region='', exec_type='GET'):
        ...
        if region == '':
            return 'region is required.'
        with self.tracer.start_as_current_span("get_vpcs") as span:
            vpcIns = VPC()
    ...
    def get_vpc(self, profile='', region='', vpc='', exec_type='GET'):
        ...
        with self.tracer.start_as_current_span("get_vpc") as span:
            vpcIns = VPC()
